...
import { formatNumberWithCommas, fromCamelCase } from '@/utils';
import ShowMoreBtn from '../ShowMoreBtn';
import cn from '@/lib/clsx/cn';

function CardComponentTwo({ productObj, className, classNameForSection, classNameForSectionDiv, classNameForProductTitle, classNameForKeyFeatureContainer, classNameForKeyFeatureKeys, classNameForSavePercentageSection, classNameForBrandSection }) {
    const { _id, brand, productTitle, imgUrls, keyFeatures, regularPrice, price } = productObj;
    return (
        <article className={cn('h-full pb-2 flex flex-col overflow-clip rounded-lg shadow-md hover:shadow-lg dark:shadow-none dark:bg-[#1e1e1e]', className)}>
            <figure className='overflow-clip relative border rounded-tl-lg rounded-tr-lg dark:border-none'>
                <small className={cn('absolute top-5 px-3 text-white font-bold bg-red-500 rounded-e-lg', classNameForSavePercentageSection)}>{`Save ${formatNumberWithCommas(regularPrice - price)} BDT` }</small>
            ...
            <section className={cn('p-2 flex-1 flex flex-col justify-between gap-5', classNameForSection)} role='region'>
                <div className={cn('flex flex-col gap-3', classNameForSectionDiv)}>
