    ...
    <Link href="">
      <article className={cn('overflow-clip shadow-md h-full flex flex-col cursor-pointer', className)}>
        <figure className='relative overflow-clip'>
          {/* offers section */}
          {
            offer.length > 0 && <span className='absolute top-2 right-2 bg-red-500 text-white font-bold border border-red-500 rounded-xl px-3'>{offer.length > 0 && offer}</span>
          }

          {/* points section */}
          {
            points > 0 && <span className={`absolute ${offer.length > 0 ? 'top-10' : 'top-2'} right-2 bg-purple-900 px-3 rounded-xl font-bold text-white`}>Earn points {points}</span>
          }
