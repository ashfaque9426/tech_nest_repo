              ...
              productDescriptionArr.map(descriptionObj => <ul className='flex flex-col gap-2' key={`descParentUL${uuidv4()}`}>
                {
                    Object.keys(descriptionObj).map(key => <li key={`descListItem${uuidv4()}`}>
                        <span className='text-xl font-semibold capitalize'>{fromCamelCase(key)}:</span> <span className={`${key === 'title' && 'font-semibold text-lg capitalize'}`}>{key === 'title' ? fromCamelCase(descriptionObj[key]) : descriptionObj[key]}</span>
                    </li>)
                }
            </ul>
