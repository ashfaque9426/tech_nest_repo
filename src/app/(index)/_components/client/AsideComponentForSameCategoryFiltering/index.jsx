...
function AsideComponentForSameCategoryFiltering({ category, brand, handleParamsForUrl }) {
    ...
    return (
        <aside className='md:w-[25%]' role="complementary" aria-labelledby="sidebar-heading">
            ...
            {
                category === "desktop processor" || category === "desktop%20processor" && <FilterationComponentForAside sidebarArr={processorSidebar} handleParamsForUrl={handleParamsForUrl} indexArr={indexArr} setIndexArr={setIndexArr} multipleObjsOfStateArr={true} />
            }

            {/* for motherboard Filtering options */}
            {
                category === "motherboard" && <FilterationComponentForAside sidebarArr={motherboardSidebar} handleParamsForUrl={handleParamsForUrl} indexArr={indexArr} setIndexArr={setIndexArr} multipleObjsOfStateArr={false} />
            }

            {/* for laptop Filtering options */}
            {
                category === "laptop" && <FilterationComponentForAside sidebarArr={laptopSidebar} handleParamsForUrl={handleParamsForUrl} indexArr={indexArr} setIndexArr={setIndexArr} multipleObjsOfStateArr={false} />
            }


            {/* for phone Filtering options */}
            {
                category === "smart%20phone" && <FilterationComponentForAside sidebarArr={phoneSidebar} handleParamsForUrl={handleParamsForUrl} indexArr={indexArr} setIndexArr={setIndexArr} multipleObjsOfStateArr={false} />
            }
