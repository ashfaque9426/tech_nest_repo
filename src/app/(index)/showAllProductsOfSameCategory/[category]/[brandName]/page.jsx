...
function Page({ params }) {
  ...
  return (
    <>
      <header>
        <h1 id='mainContentLabelHeading' className='text-2xl text-center mt-5 mb-12 font-bold capitalize'>{brandName !== 'none' ? <><span>{brandName}</span> --&gt; {category.includes('%20') && category.replace('%20', " ") || category} </> : <><span>Category:</span> {category.includes('%20') && category.replace('%20', " ") || category}</>}</h1>
      </header>
